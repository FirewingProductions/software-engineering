...
public class GameController 
{
    ...
    public void runGame(int numHumanPlayers, int numAutoPlayers) throws Exception
    {
        ...
        _gameUI = new GameUI(_gameModel);
        _gameUI.updateUI();
        while (!_gameModel.isGameOver())
        {
            Player currentPlayer = _gameModel.getCurrentPlayer();
            RuleResult result = RuleProvider.checkAtStart(_gameModel);
            for (Instruction instruction : result.getInstructions())
            {
                _gameModel.executeInstruction(instruction);
            }
            
            IUserChoice userChoice = currentPlayer.getIsAuto()? _agent : _gameUI;
            
            ArrayList<PlayerOption> options = result.getOptions();  
            if (options.size() > 0)
            {
                UserChoiceRequest choiceRequest = new UserChoiceRequest(options);
                UserChoiceResponse choiceResponse = userChoice.MakeChoice(choiceRequest);
                
                for (PlayerOptionResponse optionResponse : choiceResponse.getReponses())
                {
                    
                }
            }
            
            _gameModel.setNextPlayer();      
        }
        ...
        try
        {
            ...
            for (int x = 0; x < totalNoOfRows; x++) 
            {
                ...
                else
                {
                    ...
                    {
                        Instruction instruction = new Instruction(action, InstructionType.GoDirectTo, 0, 0, 10);
                        space = new InstructionSpace(name, instruction);
                    }
            ...
            potLuckCards.add(new Card(new Instruction("Go to jail. Do not pass GO, do not collect £200", InstructionType.GoDirectTo, 0, 0, 0)));
            ...
            opportunityCards.add(new Card(new Instruction("Go back 3 spaces", InstructionType.GoBackNumSpaces, 0, 0, 0)));
            ...
            opportunityCards.add(new Card(new Instruction("Go to jail. Do not pass GO, do not collect £200", InstructionType.GoDirectTo, 0, 0, 10)));
            ...
            gameModel.setPotLuckCards(potLuckCards);
            gameModel.setOpportunityCards(opportunityCards);
