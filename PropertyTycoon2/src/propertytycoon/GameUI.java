...
public class GameUI extends JFrame implements IUserChoice,WindowListener,ActionListener 
{
    ...
    public GameUI(GameController gameController)
    {
        ...
        this._model = gameController.getGameModel();
...
//        _gameStartPanel = new JPanel();
    ...
    public void updateUI(Boolean updateOptions)
    {
        ...
        if (updateOptions)
        {
            setOptions(_gameController.processUserResponse(new UserChoiceResponse(null)));
        }
        setVisible(true);

    }
    
    public void setOptions(UserChoiceRequest request)
    {
        options = request.getOptions();
        String[] optionTitles = new String[options.size()];
        for (int i = 0; i < options.size(); i++)
        {
            optionTitles[i] = options.get(i).getTitle();
        }
        _optionsBox = new JComboBox(optionTitles);
        _optionsBox.setSelectedIndex(0);
    }
    ...
    {
        int selectedIndex = _optionsBox.getSelectedIndex();
        UserChoiceRequest request = _gameController.processUserResponse(new UserChoiceResponse(options.get(selectedIndex)));
        updateUI(false);
        setOptions(request);
    }
    ...
    public UserChoiceResponse MakeChoice(UserChoiceRequest request)
    {
        ...
        UserChoiceResponse response = new UserChoiceResponse(request.getOptions().get(0));
